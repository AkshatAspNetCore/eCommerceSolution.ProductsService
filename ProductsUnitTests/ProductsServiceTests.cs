...
public class ProductsServiceTests
{
    ...
    public ProductsServiceTests() 
    {
        ...
        _productsService = new ProductsService(_productAddRequestValidatorMock.Object,
    ...
    public async Task AddProduct_ShouldAddValidProduct() 
    {
        ...
        _productAddRequestValidatorMock.Setup(validator => validator.ValidateAsync(It.IsAny<ProductAddRequest>(),It.IsAny<CancellationToken>())).ReturnsAsync(new FluentValidation.Results.ValidationResult());
    ...
    {
        //Arrange
        var productAddReuest = _fixture.Create<ProductAddRequest>();
        var validationResult = new FluentValidation.Results.ValidationResult(new[] { 
            new ValidationFailure("Property","Error")
        });

        _productAddRequestValidatorMock.Setup(validator => validator.ValidateAsync(It.IsAny<ProductAddRequest>(),It.IsAny<CancellationToken>())).ReturnsAsync(validationResult);

        //Act
        Func<Task> act = async () => await _productsService.AddProduct(productAddReuest);
    ...
    public async Task UpdateProduct_ShouldThrowArgumentExceptionWhenProductDoesNotExist()
    {
        ...
        // Assert
        await act.Should().ThrowAsync<ArgumentException>()
                 .WithMessage("Value cannot be null. (Parameter 'Invalid Product Id')");
